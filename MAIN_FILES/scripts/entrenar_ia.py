...
class EntornoRiegoMaiz(gym.Env):
    ...
    def step(self, action):
        ...
        # Acumulamos el coste real del agua — se descuenta íntegro en la cosecha.
        self.coste_agua_acumulado += riego_ia * 10 * precio_agua_hoy  # mm -> m3/ha

        # --- FIX 1: Penalización por eficiencia de riego ─────────────────────
        #
        # DIAGNÓSTICO (v2): el 20% diario de coste era -0.12 pts para 15mm,
        # mientras el Ks penalty era -0.40 pts. Ratio 3.3x → agente ignoraba el coste.
        # El resultado: 1,568 mm de riego (90% más que FAO-56) y peor beneficio.
        #
        # NUEVO ENFOQUE — penalización continua por ineficiencia hídrica:
        #   reward -= riego * (1 - Dr/TAW) * COEF
        #
        # Motivación física: irrigar suelo seco (Dr/TAW≈1) es eficiente → sin penalización.
        # Irrigar suelo húmedo (Dr/TAW≈0) es desperdicio → penalización máxima.
        # La penalización es PROPORCIONAL a cuán lleno está el suelo, no a un umbral fijo.
        #
        # Calibración (comparable con FIX2 para Ks=0.80, señal=-0.40 pts):
        #   - 15mm en suelo medio lleno (Dr/TAW=0.50): 15*0.50*0.025 = -0.19 pts  → moderado
        #   - 15mm en suelo casi lleno  (Dr/TAW=0.10): 15*0.90*0.025 = -0.34 pts  → similar a estrés leve
        #   - 15mm en suelo casi vacío  (Dr/TAW=0.90): 15*0.10*0.025 = -0.04 pts  → casi cero (riego necesario)
        #   - 0mm  (sin riego):          0 pts                                      → sin penalización
        if riego_ia > 0:
            taw_actual  = max(self.calc.TAW, 1.0)
            dr_taw_ratio = self.calc.Dr / taw_actual
            humedad_relativa_suelo = 1.0 - dr_taw_ratio   # 0=seco, 1=lleno
        ...
        if terminated:
            ...
            # Beneficio neto: ingresos - coste total del agua - cuota fija.
...
def plotear_curva_aprendizaje(log_dir: str, ruta_salida: str = "curva_aprendizaje.png") -> None:
    ...
    try:
        ...
        ax.set_xlabel('Pasos de entrenament', fontsize=12)
        ax.set_ylabel('Reward per episodi (pts)', fontsize=12)
        ax.set_title('Curva de aprenentatge — Agent SAC (reg blat de moro a Bustillo del Páramo)', fontsize=13)
...
if __name__ == "__main__":
    ...
    RUTA_MODELO    = "../modelos/ia_riego_maiz_sac"      # Modelo final
    RUTA_VN_MEJOR  = "../modelos/vecnormalize_mejor.pkl"   # Stats del mejor modelo
    RUTA_VN_FINAL  = "../modelos/vecnormalize_stats.pkl"   # Stats del modelo final
    RUTA_CURVA     = "../resultados/figuras/curva_aprendizaje.png"
